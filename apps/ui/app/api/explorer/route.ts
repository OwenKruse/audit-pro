...
async function fetchFeeLikeSummary(
  ...
  const j = await fetchJson<FeeSummaryApiResponse>(url, 300);
...
export async function GET(req: Request): Promise<Response> {
  try {
    ...
    const chains = splitList(searchParams.get('chain'));

    const minTvl = toNumber(searchParams.get('minTvl'));
    const maxTvl = toNumber(searchParams.get('maxTvl'));
    const minMcap = toNumber(searchParams.get('minMcap'));
    const maxMcap = toNumber(searchParams.get('maxMcap'));
    const minMcapToTvl = toNumber(searchParams.get('minMcapToTvl'));
    const maxMcapToTvl = toNumber(searchParams.get('maxMcapToTvl'));

    const minChange1d = toNumber(searchParams.get('minChange1d'));
    const maxChange1d = toNumber(searchParams.get('maxChange1d'));
    const minChange7d = toNumber(searchParams.get('minChange7d'));
    const maxChange7d = toNumber(searchParams.get('maxChange7d'));

    const sort = (searchParams.get('sort') || 'tvl').trim();
    const order = (searchParams.get('order') || 'desc').trim().toLowerCase();
    const limitRaw = toNumber(searchParams.get('limit')) ?? DEFAULT_LIMIT;
    const offsetRaw = toNumber(searchParams.get('offset')) ?? 0;

    const limit = clamp(Math.trunc(limitRaw), 1, MAX_LIMIT);
    const offset = Math.max(0, Math.trunc(offsetRaw));

    const includeFees = toBool(searchParams.get('includeFees'));
    const includeRevenue = toBool(searchParams.get('includeRevenue'));
    const dataSourceBaseUrl = process.env.LLAMA_BASE_URL || DEFAULT_BASE_URL;

    const protocols = await fetchJson<ProtocolRow[]>(`${dataSourceBaseUrl}/protocols`, 300);
