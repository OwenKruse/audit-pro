import { ContractAuditRunRequestSchema } from '@cipherscope/proto';
...
export async function POST(req: Request): Promise<Response> {
  ...
  let parsed: ReturnType<typeof ContractAuditRunRequestSchema.parse>;
  try {
    parsed = ContractAuditRunRequestSchema.parse(body);
  } catch (err) {
  ...
  try {
    const data = await client.runContractAudit(parsed);
