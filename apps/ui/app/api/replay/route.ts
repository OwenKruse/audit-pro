import { ReplayRequestSchema } from '@cipherscope/proto';
...
export async function POST(req: Request): Promise<Response> {
  ...
  let parsed: { messageId: string; overrides?: unknown };
  try {
    parsed = ReplayRequestSchema.parse(body);
  } catch (err) {
    return Response.json(
  ...
  const upstream = await fetch(`${agentHttpUrl.replace(/\/$/, '')}/replay`, {
    method: 'POST',
