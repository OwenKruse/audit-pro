import { RpcInteractionRecordRequestSchema } from '@cipherscope/proto';
...
export async function GET(req: Request): Promise<Response> {
  ...
  if (source === 'wallet' || source === 'foundry') qs.set('source', source);

  const upstream = await fetch(`${agentHttpUrl.replace(/\/$/, '')}/rpc/interactions?${qs}`, {
    cache: 'no-store',
  }).catch(() => null);
...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    parsed = RpcInteractionRecordRequestSchema.parse(body);
  } catch (err) {
    return Response.json(
  ...
  const upstream = await fetch(`${agentHttpUrl.replace(/\/$/, '')}/rpc/interactions`, {
    method: 'POST',
